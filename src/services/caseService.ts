...
export async function deleteCase(id: string): Promise<void> {
  try {
    ...
    const { error: rpcError } = await supabase.rpc('delete_case_with_dependencies', {
      case_id: id
    });

    if (rpcError) {
      console.error('Error deleting case and dependencies:', rpcError);
      throw new Error('Failed to delete case and its dependencies');
    }
  } catch (error) {
